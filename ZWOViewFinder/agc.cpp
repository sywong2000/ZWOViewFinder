...
void agc()
{
    ...
    while (!end_program)
    {
        ...
        int new_gain = std::clamp((int)(4.0 * camera::GAIN_MAX * agc_value - (3.0 * camera::GAIN_MAX)), camera::GAIN_MIN, camera::GAIN_MAX);
        ...
        int new_exposure_us = std::clamp((int)(4.0 / 3.0 * camera::EXPOSURE_MAX_US * agc_value), camera::EXPOSURE_MIN_US, camera::EXPOSURE_MAX_US
        );
