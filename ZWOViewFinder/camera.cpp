...
std::atomic_bool end_program;

// AGC enable state
std::atomic_bool agc_enabled;

// AGC outputs
std::atomic_int camera_gain;
std::atomic_int camera_exposure_us;
...
void camera::init_camera(ASI_CAMERA_INFO &CamInfo, const char *cam_name, int binning)
{
    ...
    if (num_devices <= 0)
    {
        /*_strerror("No cameras connected.");*/
    }
    else
    {
        if (cam_name == nullptr) {
            ...
            if (asi_rtn != ASI_SUCCESS) {
                //_strerror( "ASIGetCameraProperty error: %s", asi_error_str(asi_rtn));
            }

            //stderr.println ("Found %d cameras; arbitrarily selecting %s.",num_devices, CamInfo.Name);
        } else {
            ...
            if (matches.empty()) {
                //errx(1, "Found %d cameras; no camera matched \"%s\"",num_devices, cam_name);
            } else if (matches.size() != 1) {
                //warnx("Found %d cameras; %zu different cameras matched \"%s\":",num_devices, matches.size(), cam_name);
            ...
            //warnx("Found %d cameras; exactly one camera matched \"%s\": %s",num_devices, cam_name, CamInfo.Name);
        }
    }
