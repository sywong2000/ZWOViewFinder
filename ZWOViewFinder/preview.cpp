...
// disk write state
std::atomic_bool disk_file_exists;
std::atomic_bool disk_write_enabled;
...
void preview(bool color)
{
    ...
    while (!end_program)
    {
        ...
        if (disk_file_exists)
        {
            sprintf_s(window_title, "%s %.1f FPS %s", PREVIEW_WINDOW_NAME, framerate, (disk_write_enabled) ? ("writing to disk") : ("disk write paused"));
        }
        else
        {
            sprintf_s(window_title, "%s %.1f FPS", PREVIEW_WINDOW_NAME, framerate);
        }
